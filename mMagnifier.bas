...
Private Const WindowTitle = "twinBASIC Magnifier Demo"
...
Private Function RegisterHostWindowClass(hInstance As LongPtr) As Integer
    ...
    wcex.lpfnWndProc = AddressOf HostWndProc
...
Private Function SetupMagnifier(hInst As LongPtr) As BOOL
    ...
    Dim ret As BOOL = MagSetWindowTransform(hwndMag, matrix)
    
    If ret Then
        If InvertColors Then
            Dim magEffectInvert As MAGCOLOREFFECT
            With magEffectInvert 'Reminder: x,y inverted vs cpp
                .transform(0, 0) = -1.0: .transform(1, 0) = 0: .transform(2, 0) = 0: .transform(3, 0) = 0: .transform(4, 0) = 0
                .transform(0, 1) = 0: .transform(1, 1) = -1.0: .transform(2, 1) = 0: .transform(3, 1) = 0: .transform(4, 1) = 0
                .transform(0, 2) = 0: .transform(1, 2) = 0: .transform(2, 2) = -1.0: .transform(3, 2) = 0: .transform(4, 2) = 0
                .transform(0, 3) = 0: .transform(1, 3) = 0: .transform(2, 3) = 0: .transform(3, 3) = 1.0: .transform(4, 3) = 0
                .transform(0, 4) = 1.0: .transform(1, 4) = 1.0: .transform(2, 4) = 1.0: .transform(3, 4) = 0: .transform(4, 4) = 1.0
            End With
...
Private Sub UpdateMagWindow(ByVal hWnd As LongPtr, ByVal uMsg As Long, ByVal idEvent As LongPtr, ByVal dwTime As Long)
    ...
    Dim width As Long = (magWindowRect.Right - magWindowRect.Left) / MAGFACTOR
    Dim height As Long = (magWindowRect.Bottom - magWindowRect.Top) / MAGFACTOR
    ...
    #If (TWINBASIC <> 0) or (Win64 <> 0) Then
    ...
    'Force redraw.
    InvalidateRect(hwndMag, ByVal vbNullPtr, CTRUE)
End Sub
...
Private Sub GoFullScreen()
        ...
        Dim xSpan As Long = GetSystemMetrics(SM_CXSCREEN)
        Dim ySpan As Long = GetSystemMetrics(SM_CYSCREEN)
        ReleaseDC(0, hDC)

        ' // Calculate the size of system elements.
        Dim xBorder As Long = GetSystemMetrics(SM_CXFRAME)
        Dim yCaption As Long = GetSystemMetrics(SM_CYCAPTION)
        Dim yBorder As Long = GetSystemMetrics(SM_CYFRAME)

        ' // Calculate the window origin And span For full-screen mode.
        Dim xOrigin As Long = -xBorder
        Dim yOrigin As Long = -yBorder - yCaption
        xSpan += 2 * xBorder
